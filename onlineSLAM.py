...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    length = 2 * (1 + num_landmarks)
    expand_index = [i for i in range(length + 2)]
    take_index = [i for i in range(2, length + 2)]
    del expand_index[2]
    del expand_index[2]
    omega = matrix([[0.0 for i in range(length)] for j in range(length)])
    xi = matrix([[0.0] for i in range(length)])
    ...
    for i in range(len(data)):
        # expand
        omega = omega.expand(length + 2, length + 2, expand_index, expand_index)
        xi = xi.expand(length + 2, 1, expand_index, [0])
        ...
        motion_y = data[i][1][1]
        # px(last) and px(next)
        xi.value[0][0] -= motion_x / measurement_noise
        xi.value[2][0] += motion_x / measurement_noise
        # py(last) and py(next)
        xi.value[1][0] -= motion_y / measurement_noise
        xi.value[3][0] += motion_y / measurement_noise
        ...
        for j in range(len(landmarks)):
            ...
            # px(last) and lx(k)
            omega.value[0][0] += 1 / motion_noise
            omega.value[2 * (2 + k)][2 * (2 + k)] += 1 / motion_noise
            omega.value[0][2 * (2 + k)] -= 1 / motion_noise
            omega.value[2 * (2 + k)][0] -= 1 / motion_noise
            # py(last) and ly(k)
            omega.value[1][1] += 1 / motion_noise
            omega.value[2 * (2 + k) + 1][2 * (2 + k) + 1] += 1 / motion_noise
            omega.value[1][2 * (2 + k) + 1] -= 1 / motion_noise
            omega.value[2 * (2 + k) + 1][1] -= 1 / motion_noise

            # xi
            measure_x = landmarks[j][1]
            measure_y = landmarks[j][2]
            # px(last) and lx(k)
            xi.value[0][0] -= measure_x / measurement_noise
            xi.value[2 * (2 + k)][0] += measure_x / measurement_noise
            # py(last) and ly(k)
            xi.value[1][0] -= measure_y / measurement_noise
            xi.value[2 * (2 + k) + 1][0] += measure_y / measurement_noise

        # take
        omega_prime = omega.take(take_index, take_index)
        xi_prime = xi.take(take_index, [0])
        A = omega.take([0, 1], take_index)
