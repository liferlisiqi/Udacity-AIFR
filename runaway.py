...
# The OTHER variable that your function returns will be
# passed back to your function the next time it is called.
# You can use this to keep track of important information over time.
def estimate_next_pos(measurement, heading, turning, distance):
    """Estimate the next (x, y) position of the wandering Traxbot
    based on noisy (x, y) measurements."""
    estimate_bot = robot(measurement[0], measurement[1], heading, turning, distance)
    estimate_bot.move_in_circle()

    return (estimate_bot.x, estimate_bot.y)
...
def demo_grading(estimate_next_pos_fcn, target_bot, OTHER=None):
    ...
    while not localized and ctr <= 10:
        ...
        position_guess = estimate_next_pos_fcn(measurement, target_bot.heading, target_bot.turning, target_bot.distance)
