...
def run(grid, goal, spath, params, printflag=False, speed=0.1, timeout=1000):
    ...
    while not myrobot.check_goal(goal) and N < timeout:
        ...
        detx = spath[index + 1][0] - spath[index][0]
        dety = spath[index + 1][1] - spath[index][1]
        u = (rx * detx + ry * dety) / (detx**2 + dety**2)
        if u > 1:
            index += 1
            #continue
        # here comes the question, if not stop this loop, then the cte computed
        # is based on the wrong path, when the path is changed in the next loop
        # the new cte is based on the new path, can we get diff_cte just by
        # diff_cte = cte(old) - cte(new)?

        cte = (ry * detx - rx * dety) / (detx**2 + dety**2)
...
def twiddle(init_params):
    ...
    # the average best_error computed by 10 times of same experiment
    best_error = 0.0;
    for k in range(K):
