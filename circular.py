...
class robot:
    ...
    def move(self, motion):  # Do not change the name of this function
        #my code need exception judge
        #however my code is the same as his, but the answer is different
        # ADD CODE HERE
        result = robot(length)
        ...
        if abs(beta) < 0.001:
            result.set(self.x + motion[1] * cos(self.orientation),
                       ...
                       (self.orientation + motion[0]) % (2 * pi))
        else:
            R = motion[1] / beta
            cx = self.x - R * sin(self.orientation)
            cy = self.y + R * cos(self.orientation)
            result.set(cx + R * sin(self.orientation + beta),
                       cy - R * cos(self.orientation + beta),
                       (self.orientation + beta) % (2 * pi))
...
# # motion[0] is steering angle alpha
# T = len(motions)
#
# print 'Robot:    ', myrobot
# for t in range(T):
